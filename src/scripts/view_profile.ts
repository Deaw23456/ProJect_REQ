...
function loadProfileFromUrl(): void {
    ...
    if (profileImg) profileImg.src = userProfile.profileImg || DEFAULT_PROFILE_IMG;
    if (profileBackgroundImg) profileBackgroundImg.src = userProfile.profileBackgroundImg || DEFAULT_BG_IMG;
...
function initReviewSystem(): void {
    let currentRating = 0;
    ...
    const reviewInput = document.getElementById('review-text') as HTMLTextAreaElement | null;
    const reviewListArea = document.getElementById('review-list');

    if (!ratingText || !submitBtn || !reviewInput || !reviewListArea) return;
...
function renderMiniProfiles(): void {
