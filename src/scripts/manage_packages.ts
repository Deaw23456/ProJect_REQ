...
let packagesData: Package[] = JSON.parse(localStorage.getItem(PACKAGES_DB_KEY) || 'null') || [
    { 
        name: "Basic Starter", 
        price: 4500, 
        sessions: 10, 
...
function openModal(): void {
    editingIndex = -1; 
    (document.getElementById('modal-title') as HTMLElement).innerText = "Add New Package";
    ...
    (document.getElementById('pkg-desc') as HTMLInputElement).value = '';

    showModalUI();
}

function editPackage(index: number): void {
    editingIndex = index; 
    (document.getElementById('modal-title') as HTMLElement).innerText = "Edit Package";
    
    const pkg = packagesData[index]!;
    (document.getElementById('pkg-name') as HTMLInputElement).value = pkg.name;
    (document.getElementById('pkg-price') as HTMLInputElement).value = pkg.price.toString();
    (document.getElementById('pkg-sessions') as HTMLInputElement).value = pkg.sessions.toString();
    (document.getElementById('pkg-desc') as HTMLInputElement).value = pkg.desc;
...
function savePackage(): void {
    ...
    const price = (document.getElementById('pkg-price') as HTMLInputElement).value;
    const sessions = (document.getElementById('pkg-sessions') as HTMLInputElement).value;
    const desc = (document.getElementById('pkg-desc') as HTMLInputElement).value;
    ...
    const newPkg: Package = { name, price, sessions, desc, popular: false };
...
function deletePackage(index: number): void {
...
const modal = document.getElementById('package-modal') as HTMLElement;
const modalContent = document.getElementById('modal-content') as HTMLElement;

function showModalUI(): void {
...
(window as any).editPackage = editPackage;
(window as any).savePackage = savePackage;
(window as any).deletePackage = deletePackage;
(window as any).saveAndExit = saveAndExit;
