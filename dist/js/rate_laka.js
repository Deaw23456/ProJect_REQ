...
let currentPlanName = "15 Days Challenge";
...
function updateUI() {
    ...
    if (progressLabel)
        progressLabel.innerText = `${count} / ${maxQuota} วัน`;
    if (progressBar)
        progressBar.style.width = `${(count / maxQuota) * 100}%`;
    if (badgeCount)
...
function goToPayment() {
    ...
    if (summaryTotal)
        summaryTotal.innerText = `฿${currentPrice.toLocaleString()}`;
    selectedDates.sort((a, b) => new Date(a).getTime() - new Date(b).getTime());
...
renderCalendar();
initNav();
//# sourceMappingURL=../../data/rate_laka.js.map